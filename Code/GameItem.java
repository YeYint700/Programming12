...
public class GameItem {
    ...
    public static class GameContext{
        ...
        public int hp =3;
        public int maxhp =3;
        //左から順にハートのImage（３個）を指定
        public List<ImageView> hpHearts;
        //SPEED
    ...
    //BEEF:HPを１回復
    //ハートは左から順に表示される
    private void applyBeef(GameContext ctx){
        if(ctx.hp<ctx.maxhp){
            ctx.hp+=1;
            if(ctx.hp>ctx.maxhp)
                ctx.hp = ctx.maxhp;
            updateHpHearts(ctx);
            System.out.println("HP +1 ("+ctx.hp+"/"+ctx.maxhp+")");
        } else{
            //満タンなら何もおこらない
            System.out.println("HP is already full. No effect.");
        }
    }
    ...
    //左からHPを表示
    //壁衝突で右側から消えていく
    public static void updateHpHearts(GameContext ctx){
        if(ctx.hpHearts==null || ctx.hpHearts.isEmpty())
            return;
        for(int i =0; i<ctx.hpHearts.size(); i++){
            boolean visible = i <  ctx.hp;
            ctx.hpHearts.get(i).setVisible(visible);
        }
    }
    ...
    //右から一つ消える
    //０未満にはならない
    public static boolean onWallDamage(GameContext ctx){
        if(ctx.hp>0)
            ctx.hp-=1;
        if(ctx.hp<0)
            ctx.hp=0;
        updateHpHearts(ctx);
        System.out.println("Wall Damage: HP -1 ("+ctx.hp+"/"+ctx.maxhp+")");
        return ctx.hp=-0;
    }
